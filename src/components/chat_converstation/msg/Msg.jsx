...
export const Msg = ({ showMenu = true, showTime = true }) => {
  ...
  const getImagesAndVideos = useMemo(() => {
    // const messagesWithFiles = Object.values(msgs).filter((item) => item?.files);
    const messagesWithFiles = Object.values(msgs);
    // .map((item) => item?.files);
    // const filtered = messagesWithFiles.pop();
    console.log(messagesWithFiles);
    for (const f of messagesWithFiles) {
    }
  }, [room_id, msgs]);
